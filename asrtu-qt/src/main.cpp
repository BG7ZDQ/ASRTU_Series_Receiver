...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (arguments.contains(QStringLiteral("--maximized")))
        ...
        if (argc >= 3 && QString::fromLocal8Bit(argv[1]) == QStringLiteral("--screenshot")) {
            ...
            if (argc >= 4 && QString::fromLocal8Bit(argv[3]) == QStringLiteral("--loop")) {
                if (auto* tabs = window.findChild<QTabWidget*>(); tabs && tabs->count() > 1)
